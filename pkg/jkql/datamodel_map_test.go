...
func TestNamedMapKeys(t *testing.T) {
	...
	cases := []struct {
		...
		{"Properties('a,b')", []string{"a,b"}},       // a comma inside a quoted key is not a separator
...
func TestBuildDataModel_NeverExplodeMapFieldStaysRaw(t *testing.T) {
	// A whole-map access on a neverExplodeMapFields entry (Statistics) stays a single raw
	// column; any other map field would explode per key by default.
	raw := `{
		"row-count": 1, "total-row-count": 1, "status": "SUCCESS",
		"colhdr": ["Statistics"],
...
func TestSplitArgs(t *testing.T) {
	...
	cases := []struct {
		...
		{"Quota('a,b'),0.95", []string{"Quota('a,b')", "0.95"}}, // comma inside quotes is not a separator
		{"'a','b'", []string{"'a'", "'b'"}},                     // separators between quoted args still split
		{"plain", []string{"plain"}},                            // a single arg with no comma
	}
	for _, c := range cases {
...
// TestBuildDataModel_QuotedCommaKeyExplodesOnce pins that a Properties key whose name contains a
// comma (Properties('a,b')) explodes into exactly one column for the real key "a,b", not into
// phantom all-null "a"/"b" columns — the comma inside the quoted key must not be read as a key
// separator.
func TestBuildDataModel_QuotedCommaKeyExplodesOnce(t *testing.T) {
	raw := `{
		"row-count": 1, "total-row-count": 1, "status": "SUCCESS",
		"colhdr": ["Properties('a,b')"],
		"coltype": {"Properties('a,b')": "MAP"},
		"collabel": {"Properties('a,b')": "a,b"},
		"rows": [ {
			"Properties('a,b')": {"a,b": "v"},
			"Properties('a,b'):_ValueTypes": {"a,b": "STRING"}
		} ]
	}`
	m := BuildDataModel(parseRS(t, raw), nil)

	if len(m.Headers) != 1 {
		t.Fatalf("want exactly 1 exploded column for key \"a,b\", got %d: %v", len(m.Headers), m.Headers)
	}
	header := m.Headers[0]
	if m.Rows[0][header] != "v" {
		t.Errorf("value = %v, want v (the real \"a,b\" key's value)", m.Rows[0][header])
	}
}

// TestBuildDataModel_TypedMapFunctionAllNullKeepsColumn pins that a typed-map function result
// (Concat(Statistics), MAP(STRING)) that is null on every row still emits its column — the column
// must not vanish on an all-null or 0-row refresh, or a table panel loses its schema.
func TestBuildDataModel_TypedMapFunctionAllNullKeepsColumn(t *testing.T) {
	raw := `{
		"row-count": 1, "total-row-count": 1, "status": "SUCCESS",
		"colhdr": ["Concat(Statistics)"],
		"coltype": {"Concat(Statistics)": "MAP(STRING)"},
		"collabel": {"Concat(Statistics)": "Combined Stats"},
		"rows": [ {"Concat(Statistics)": null} ]
	}`
	m := BuildDataModel(parseRS(t, raw), nil)

	if len(m.Headers) != 1 {
		t.Fatalf("want 1 fallback column, got %d: %v", len(m.Headers), m.Headers)
	}
	header := m.Headers[0]
	if header != "S:Concat(Statistics)" {
		t.Errorf("header = %q, want %q", header, "S:Concat(Statistics)")
	}
	if m.DataTypes[header] != STRING {
		t.Errorf("type = %q, want STRING", m.DataTypes[header])
	}
	if m.Label[header] != "Combined Stats" {
		t.Errorf("label = %q, want the server label kept", m.Label[header])
	}
}

// TestBuildDataModel_WholeMapAggregateAllNullEmitsNoColumn pins that a whole-map aggregate
// (Avg(Quota)) with no data on any row emits zero columns — the backfill only preserves a NAMED
// key. Previously it emitted a phantom Fn(Map(”)) column labeled "Avg()".
func TestBuildDataModel_WholeMapAggregateAllNullEmitsNoColumn(t *testing.T) {
	raw := `{
		"row-count": 1, "total-row-count": 1, "status": "SUCCESS",
		"colhdr": ["Avg(Quota)"],
		"coltype": {"Avg(Quota)": "MAP(DECIMAL)"},
	...
	if len(m.Headers) != 0 {
		t.Errorf("headers = %v, want none (a whole-map aggregate with no data drops)", m.Headers)
	}
}

// TestBuildDataModel_NamedKeyAggregateAllNullKeepsColumn is the counter-case: a NAMED-key aggregate
// (Avg(Quota('X'))) with no data must still emit exactly one column, so a named request keeps a
// stable schema across an empty refresh.
func TestBuildDataModel_NamedKeyAggregateAllNullKeepsColumn(t *testing.T) {
	raw := `{
		"row-count": 1, "total-row-count": 1, "status": "SUCCESS",
		"colhdr": ["Avg(Quota('X'))"],
		"coltype": {"Avg(Quota('X'))": "MAP(DECIMAL)"},
		"collabel": {"Avg(Quota('X'))": "Avg X"},
		"rows": [ {"Avg(Quota('X'))": null} ]
	}`
	m := BuildDataModel(parseRS(t, raw), nil)

	if len(m.Headers) != 1 || m.Headers[0] != "D:Avg(Quota('X'))" {
		t.Fatalf("headers = %v, want [D:Avg(Quota('X'))]", m.Headers)
	}
}

// TestBuildDataModel_MixedMapFunctionNamedKeyTypeSuffix pins that a non-aggregate function over a
// named map key (Length(Properties('a'))) whose key holds different types across rows gets the
// " (Type)" label suffix in the named-key branch too, so the two columns don't share one label.
func TestBuildDataModel_MixedMapFunctionNamedKeyTypeSuffix(t *testing.T) {
	raw := `{
		"row-count": 2, "total-row-count": 2, "status": "SUCCESS",
		"colhdr": ["Length(Properties('a'))"],
		"coltype": {"Length(Properties('a'))": "MAP"},
		"collabel": {"Length(Properties('a'))": "A Length"},
		"rows": [
			{"Length(Properties('a'))": {"a": 7}, "Length(Properties('a')):_ValueTypes": {"a": "INTEGER"}},
			{"Length(Properties('a'))": {"a": "seven"}, "Length(Properties('a')):_ValueTypes": {"a": "STRING"}}
		]
	}`
	m := BuildDataModel(parseRS(t, raw), nil)

	want := []string{"A Length (Integer)", "A Length (String)"}
	if got := labelsOf(m); !reflect.DeepEqual(got, want) {
		t.Errorf("labels = %v, want %v (each type disambiguated even for a named key)", got, want)
	}
}
