...
// TestBuildDataFrame_EnumColumn_CompactKeyedByOrdinalAndName pins that the no-resolver fallback
// keys its compact table by the whole (ordinal, name) pair, not the ordinal alone. Two malformed
// wire values ("garbageA", "garbageB") both parse to ordinal 0; keying by ordinal only would give
// them one shared slot and make row 2 display row 1's name.
func TestBuildDataFrame_EnumColumn_CompactKeyedByOrdinalAndName(t *testing.T) {
	model := BuildDataModel(enumResultSet("garbageA", "garbageB"), nil)
	resolver := func(string) []string { return nil } // no resolved table -> fallback path
	...
	if !seen["garbageA"] || !seen["garbageB"] {
		t.Errorf("Text table = %v, want both names present", enumConfig.Text)
	}

	// Each row must display its own name, not both collapsing onto the first.
	i0 := *(field.At(0).(*data.EnumItemIndex))
	i1 := *(field.At(1).(*data.EnumItemIndex))
	if enumConfig.Text[i0] != "garbageA" {
		t.Errorf("row 0 displays %q, want garbageA", enumConfig.Text[i0])
	}
	if enumConfig.Text[i1] != "garbageB" {
		t.Errorf("row 1 displays %q, want garbageB", enumConfig.Text[i1])
	}
}

// TestBuildDataFrame_EnumColumn_OrdinalOutsideResolvedTableRecordsIssue pins that, on the resolved
// (dense) path, an enum ordinal outside the resolved value table leaves the cell null AND records a
// parse issue mentioning the ordinal — instead of silently nulling it. A null enum cell (normal)
// must NOT record an issue.
func TestBuildDataFrame_EnumColumn_OrdinalOutsideResolvedTableRecordsIssue(t *testing.T) {
	parsed := enumResultSet("5#WAYOUT", "")
	parsed["rows"] = []interface{}{
		map[string]interface{}{"Severity": "5#WAYOUT"}, // ordinal 5 is past a 2-entry table
		map[string]interface{}{"Severity": nil},        // a null cell must not record an issue
	}
	model := BuildDataModel(parsed, nil)
	resolver := func(string) []string { return []string{"INFO", "ERROR"} } // only ordinals 0-1

	frame := BuildDataFrame(model, resolver)
	field := frame.Fields[0]

	if field.At(0) != (*data.EnumItemIndex)(nil) {
		t.Errorf("row 0 (out-of-range ordinal) = %v, want a null cell", field.At(0))
	}

	issues := model.Issues.List()
	foundOrdinal := false
	for _, issue := range issues {
		if strings.Contains(issue, "5") {
			foundOrdinal = true
		}
	}
	if !foundOrdinal {
		t.Errorf("issues = %v, want one mentioning the out-of-range ordinal 5", issues)
	}
}
