...
// itemField identifies one field within one item type. The lookup upper-cases only the probe, so
// rule keys must be written upper-case or they never match.
