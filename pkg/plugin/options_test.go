...
func TestBuildMeshIqDataSourceOptions_MissingServiceUrlFails(t *testing.T) {
	...
	if err.Error() != "invalid service URL" {
		t.Errorf("error = %q, want %q", err.Error(), "invalid service URL")
	}
}
...
func TestBuildMeshIqDataSourceOptions_MissingTokenFails(t *testing.T) {
	...
	if err.Error() != "invalid access token" {
		t.Errorf("error = %q, want %q", err.Error(), "invalid access token")
	}
}
...
func TestValidateMeshIqDataSourceOptions(t *testing.T) {
	tests := []struct {
		...
		{"missing service URL", MeshIqDataSourceOptions{Token: "t"}, "invalid service URL"},
		{"missing token", MeshIqDataSourceOptions{ServiceUrl: "http://x"}, "invalid access token"},
		{"missing both (URL checked first)", MeshIqDataSourceOptions{}, "invalid service URL"},
	}
