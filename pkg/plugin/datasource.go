...
// errCrossHostRedirect makes http.Client abort a redirect whose target host or scheme differs
// from the original request's. Go's default policy only strips Authorization/Cookie on a
// cross-host hop; a custom header like X-API-Key (HDR_API_KEY in protocol.go) would be forwarded
// unchanged to whatever host (or, without the scheme check, an http:// downgrade on the same host)
// a redirect points at — a misconfigured Service URL, or a proxy/login page.
var errCrossHostRedirect = errors.New("refusing to follow a redirect to a different host or scheme")
...
func (d *Datasource) query(ctx context.Context, pCtx backend.PluginContext, query backend.DataQuery) backend.DataResponse {
	...
	if err != nil {
		...
		logger.Warn("could not parse the query model", "error", err)
		return backend.ErrDataResponse(backend.StatusBadRequest, "invalid query: could not parse the request")
	}
	...
	if err != nil {
		logger := log.DefaultLogger.FromContext(ctx)
		logger.Warn("datasource is not configured correctly", "error", err)
		return backend.ErrDataResponse(backend.StatusBadRequest, "datasource is not configured correctly")
	}
