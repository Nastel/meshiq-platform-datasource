...
export function registerJkqlLanguage(monaco: Monaco): void {
  ...
  monaco.languages.registerCompletionItemProvider(JKQL_LANGUAGE_ID, {
    ...
    provideCompletionItems: async (model, position, _context, token) => {
      ...
      // The active resolver can change (or go null, e.g. the owning editor unmounted) while the
      // debounce was waiting — re-check rather than trusting the reference captured above the await.
      if (token.isCancellationRequested || !activeResolver) {
        return { suggestions: [] };
      }
